describe('58289 Administrar Etapa del Proceso electoral - Consulta de etapa proceso electoral', () => {
    ...
    it('CP01_Acceso a la pantalla "Administrar Etapa del Proceso Electoral', () => {
    ...
    it('CP02_Visualización inicial de la grilla y sección de totales', () => {
        cy.contains('Administrar Etapa Proceso Electoral').click()
        cy.url().should('eq', 'https://celerisawsqa.tps.net.co/dashboard/parameterization/manage-stages');
        cy.screenshot('Paso 2 Administrar etapa', { capture: 'runner' });
    ...
    it('CP03_Validación Búsqueda de etapas (consultas exitosas, sin resultados y campo obligatorio)', () => {
        cy.contains('Administrar Etapa Proceso Electoral').click()
        cy.url().should('eq', 'https://celerisawsqa.tps.net.co/dashboard/parameterization/manage-stages');
        cy.screenshot('Paso 2 Administrar etapa', { capture: 'runner' });
    ...
    it('CP04_Consulta exitosa con filtro aplicado', () => {
        cy.contains('Administrar Etapa Proceso Electoral').click()
        cy.url().should('eq', 'https://celerisawsqa.tps.net.co/dashboard/parameterization/manage-stages');
        cy.get('input[placeholder="Nombre etapa"]').dblclick({force: true})
        cy.get('input[placeholder="Nombre etapa"]').type('Automatización etapa')
    ...
    it('CP05_Funcionalidad del botón Limpiar', () => {
        cy.contains('Administrar Etapa Proceso Electoral').click()
        cy.url().should('eq', 'https://celerisawsqa.tps.net.co/dashboard/parameterization/manage-stages');
        cy.get('input[placeholder="Nombre etapa"]').dblclick({force: true})
        cy.get('input[placeholder="Nombre etapa"]').type('Automatización etapa')
    ...
    it('CP06_Acciones en la grilla, creación y log de auditoría', () => {
        ...
        cy.get('.text-sm.text-gray-500.mt-2.px-4.font-paragraph').should('be.visible').and('contain.text','¿Está seguro de que desea eliminar esta etapa? Esta acción no se puede deshacer.')
    })
})